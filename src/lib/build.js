...
if (process.platform == "win32") {
  ...
  exec(
    msbuildPath +
      ' ./ta_lib.sln /p:FrameworkPathOverride="' +
      frameworkVersion +
      '" /property:Configuration=csr /property:Platform=' +
      arch,
  ...
  if (fs.existsSync("/usr/local/lib/libta_lib.a")) {
    console.log(
      "package devel/ta-lib is installed. No need to build talib functions."
    );
  } else {
    console.error(
      "Please install ta-lib from ports collection: pkg install devel/ta-lib"
    );
  }
} else {
  var flags = "";
  if (process.platform == "darwin") {
    var arch =
      process.arch == "ia32"
        ...
        : process.arch;
    flags =
      'MACOSX_DEPLOYMENT_TARGET=10.7 export CFLAGS="-arch ' +
      arch +
      '" && export LDFLAGS="-arch ' +
      arch +
      '" && ';
